...
def _parse_git_iso_datetime(s: str) -> datetime:
    # `git show --format=%cI` can end with 'Z'. Python's fromisoformat doesn't accept it.
    s = s.strip()
    if s.endswith("Z"):
        s = s[:-1] + "+00:00"
    ...
    if use_cache and os.path.exists("models_info.json"):
        ...
        for model_path in model_paths:
            ...
            if model in models_info:
                continue
            commits = _git("log", "--diff-filter=A", "--format=commit %H", "--", model_path).split("\n")
            commit_hash = _extract_commit_hash(commits)
            committed_datetime = _parse_git_iso_datetime(_git("show", "-s", "--format=%cI", commit_hash))

...
def _git(*args: str, cwd: Path = PATH_TO_REPO, check: bool = True) -> str:
...
def _diff_paths_starting_with(prefix: str, *git_args: str) -> set[str]:
    out = _git(*git_args, cwd=PATH_TO_REPO)
    return {p for p in out.splitlines() if p and p.startswith(prefix)}
...
def find_matching_model_files(check_all: bool = False):
    ...
    if not check_all:
        ...
        # Unstaged changes
        for p in _diff_paths_starting_with("src/transformers", "diff", "--name-only"):
            module_diff_files.add(os.path.join(PATH_TO_REPO, p))
        # Staged changes
        for p in _diff_paths_starting_with("src/transformers", "diff", "--name-only", "--cached"):
            module_diff_files.add(os.path.join(PATH_TO_REPO, p))
        # Changes in current HEAD compared to main
        main_ref = _main_ref(PATH_TO_REPO)
        for p in _diff_paths_starting_with("src/transformers", "diff", "--name-only", f"{main_ref}..HEAD"):
            module_diff_files.add(os.path.join(PATH_TO_REPO, p))
...
def check_docstrings(overwrite: bool = False, check_all: bool = False):
    ...
    if not check_all:
        module_diff_files = set()
        module_diff_files.update(_diff_paths_starting_with("src/transformers", "diff", "--name-only"))
        module_diff_files.update(_diff_paths_starting_with("src/transformers", "diff", "--name-only", "--cached"))
        main_ref = _main_ref(PATH_TO_REPO)
        module_diff_files.update(_diff_paths_starting_with("src/transformers", "diff", "--name-only", f"{main_ref}..HEAD"))
        # quick escape route: if there are no module files in the diff, skip this check
        if len(module_diff_files) == 0:
            return

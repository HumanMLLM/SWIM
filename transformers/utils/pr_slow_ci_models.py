...
def get_new_python_files_between_commits(base_commit: str, commits: list[str]) -> list[str]:
    ...
    for commit in commits:
        out = _git("diff", "--name-status", "-M", f"{commit}..{base_commit}", cwd=PATH_TO_REPO)
        for line in out.splitlines():
            if not line:
                continue
            status, *paths = line.split("\t")
            if status == "A" and paths and paths[0].endswith(".py"):
                code_diff.append(paths[0])
    return code_diff
...
def get_new_python_files(diff_with_last_commit=False) -> list[str]:
    ...
    if not diff_with_last_commit:
        print(f"main is at {main_sha}")
        print(f"Current head is at {head}")

        commits = _git("merge-base", "--all", main_ref, "HEAD", cwd=repo_dir).splitlines()
        for commit in commits:
            print(f"Branching commit: {commit}")
    else:
        print(f"main is at {main_sha}")
        commits = _parents(repo_dir, main_ref)
        for commit in commits:
            print(f"Parent commit: {commit}")

    return get_new_python_files_between_commits(head, commits)

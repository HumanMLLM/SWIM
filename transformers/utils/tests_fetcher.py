...
def _git(*args: str, cwd: Path = PATH_TO_REPO, check: bool = True, strip: bool = True) -> str:
    """Run `git` and return stdout.

    This module intentionally uses the git CLI to avoid the GitPython dependency.
    """
...
def _main_ref(cwd: Path = PATH_TO_REPO) -> str:
    """Return the best-guess main ref name in this repo ("main" or "origin/main")."""
    ...
    return "main" if res.returncode == 0 else "origin/main"


def _head_sha(cwd: Path = PATH_TO_REPO) -> str:
    return _git("rev-parse", "HEAD", cwd=cwd)


def _current_ref_for_restore(cwd: Path = PATH_TO_REPO) -> str:
    # On non-detached heads, restore by branch name; otherwise restore by SHA.
    res = subprocess.run(
    ...
    return branch if branch else _head_sha(cwd)


def _merge_bases(cwd: Path, ref1: str, ref2: str) -> list[str]:
    out = _git("merge-base", "--all", ref1, ref2, cwd=cwd)
    return [x for x in out.splitlines() if x]


def _parents(cwd: Path, commit: str) -> list[str]:
    # Output: <commit> <parent1> <parent2> ...
    out = _git("rev-list", "--parents", "-n", "1", commit, cwd=cwd)
    parts = out.split()
    return parts[1:]


def _name_status_between(cwd: Path, old: str, new: str) -> list[tuple[str, str, str]]:
    """Return parsed name-status entries between two commits.

    Returns tuples of (change_type, a_path, b_path). For non-renames, a_path == b_path.
    """

    out = _git("diff", "--name-status", "-M", f"{old}..{new}", cwd=cwd)
    ...
    for line in out.splitlines():
        ...
        if status.startswith("R") and len(parts) >= 3:
            ...
            entries.append((change_type, path, path))
    return entries
...
@contextmanager
def checkout_commit(repo_dir: Union[str, Path], commit_id: str):
    """
    Context manager that checks out a given commit when entered, but gets back to the reference it was at on exit.

    Args:
        repo_dir (`str` or `Path`): Path to a git repository.
        commit_id (`str`): The commit reference to checkout inside the context manager.
    """
    repo_dir = Path(repo_dir)
    current_head = _current_ref_for_restore(repo_dir)

    try:
        _git("checkout", commit_id, cwd=repo_dir)
        yield

    finally:
        _git("checkout", current_head, cwd=repo_dir)
...
def diff_is_docstring_only(repo_dir: Union[str, Path], branching_point: str, filename: str) -> bool:
    """
    ...
    """
    repo_dir = Path(repo_dir)
    # Read old content from git without mutating the working tree.
    old_content = _git("show", f"{branching_point}:{filename}", cwd=repo_dir, strip=False)
    with open(repo_dir / filename, "r", encoding="utf-8") as f:
        new_content = f.read()
...
def diff_contains_doc_examples(repo_dir: Union[str, Path], branching_point: str, filename: str) -> bool:
    """
...
def get_impacted_files_from_tiny_model_summary(diff_with_last_commit: bool = False) -> list[str]:
    ...
    if not diff_with_last_commit:
        main_ref = _main_ref(repo_dir)
        print(f"main is at {_git('rev-parse', main_ref, cwd=repo_dir)}")
        print(f"Current head is at {head}")

        commits = _merge_bases(repo_dir, main_ref, "HEAD")
        for commit in commits:
            print(f"Branching commit: {commit}")
    else:
        print(f"main is at {head}")
        commits = _parents(repo_dir, "HEAD")
        for commit in commits:
            print(f"Parent commit: {commit}")

    if not os.path.isfile(repo_dir / "tests/utils/tiny_model_summary.json"):
        return []
    ...
    for commit in commits:
        old_content = _git(
            "show",
            f"{commit}:tests/utils/tiny_model_summary.json",
            cwd=repo_dir,
            strip=False,
        )
        with open(repo_dir / "tests/utils/tiny_model_summary.json", "r", encoding="utf-8") as f:
            new_content = f.read()
...
def get_diff(repo_dir: Union[str, Path], base_commit: str, commits: list[str]) -> list[str]:
    """
    ...
    code_diff: list[str] = []
    for commit in commits:
        for change_type, a_path, b_path in _name_status_between(repo_dir, commit, base_commit):
            if change_type == "A" and b_path.endswith(".py"):
                code_diff.append(b_path)
            elif change_type == "D" and a_path.endswith(".py"):
                code_diff.append(a_path)
            elif change_type in ["M", "R"] and b_path.endswith(".py"):
                if a_path != b_path:
                    code_diff.extend([a_path, b_path])
                else:
                    if diff_is_docstring_only(repo_dir, commit, b_path):
                        print(f"Ignoring diff in {b_path} as it only concerns docstrings or comments.")
                    else:
                        code_diff.append(a_path)
...
def get_modified_python_files(diff_with_last_commit: bool = False) -> list[str]:
    ...
    if not diff_with_last_commit:
        ...
        branching_commits = _merge_bases(repo_dir, main_ref, "HEAD")
        for commit in branching_commits:
            print(f"Branching commit: {commit}")
        return get_diff(repo_dir, head, branching_commits)
    else:
        print(f"main is at {head}")
        parent_commits = _parents(repo_dir, "HEAD")
        for commit in parent_commits:
            print(f"Parent commit: {commit}")
        return get_diff(repo_dir, head, parent_commits)


def get_diff_for_doctesting(repo_dir: Union[str, Path], base_commit: str, commits: list[str]) -> list[str]:
    """
    ...
    for commit in commits:
        for change_type, a_path, b_path in _name_status_between(repo_dir, commit, base_commit):
            if not (b_path.endswith(".py") or b_path.endswith(".md")):
                continue
            if change_type == "A":
                code_diff.append(b_path)
                ...
                if a_path != b_path:
                    ...
                    if diff_contains_doc_examples(repo_dir, commit, b_path):
                        code_diff.append(a_path)
                    else:
                        print(f"Ignoring diff in {b_path} as it doesn't contain any doc example.")
    return code_diff
...
def get_new_doctest_files(repo_dir: Union[str, Path], base_commit: str, branching_commit: str) -> list[str]:
    """
    ...
    changed = _git(
        "diff",
        "--name-only",
        f"{branching_commit}..{base_commit}",
        "--",
    ...
    old_content = _git(
        "show",
        f"{branching_commit}:utils/not_doctested.txt",
        cwd=repo_dir,
        strip=False,
    )
    with open(repo_dir / "utils/not_doctested.txt", "r", encoding="utf-8") as f:
        new_content = f.read()
...
def get_doctest_files(diff_with_last_commit: bool = False) -> list[str]:
    ...
    if not diff_with_last_commit:
        ...
        for commit in branching_commits:
            print(f"Branching commit: {commit}")
        test_files_to_run = get_diff_for_doctesting(repo_dir, head, branching_commits)
    else:
        print(f"main is at {head}")
        parent_commits = _parents(repo_dir, "HEAD")
        for commit in parent_commits:
            print(f"Parent commit: {commit}")
        test_files_to_run = get_diff_for_doctesting(repo_dir, head, parent_commits)

    all_test_files_to_run = get_all_doctest_files()

    # Add to the test files to run any removed entry from "utils/not_doctested.txt".
    main_ref = _main_ref(repo_dir)
    main_sha = _git("rev-parse", main_ref, cwd=repo_dir)
    new_test_files = get_new_doctest_files(repo_dir, head, main_sha)
...
if __name__ == "__main__":
    ...
    if args.print_dependencies_of is not None:
        ...
        repo_dir = PATH_TO_REPO
        commit_message = _git("log", "-1", "--format=%B", cwd=repo_dir)

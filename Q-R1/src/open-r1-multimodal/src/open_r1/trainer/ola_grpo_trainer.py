...
class OlaGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model = model_cls.from_pretrained(model_id, **model_init_kwargs, low_cpu_mem_usage=False)
        # print(model_init_kwargs)
        # print(model)
                # Processing class
        ...
        if is_deepspeed_zero3_enabled() or is_deepspeed_available():
            self.ref_model = model_cls.from_pretrained(model_id, **model_init_kwargs, low_cpu_mem_usage=False)
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, completion_ids, **custom_multimodal_inputs):
        # print(custom_multimodal_inputs)
        ...
        # if completion_ids_length is not None:
        #     logits = logits[:, -completion_ids_length:, :]  # (B, L-1, V), exclude the last logit: it corresponds to the next token pred
        #     input_ids = input_ids[:, -completion_ids_length:]  # (B, L-1), exclude the first input ID since we don't have logits for it

        # Compute the log probabilities for the input tokens. Use a loop to reduce memory peak.
        per_token_logps = []
        for i, (logits_row, input_ids_row) in enumerate(zip(logits, input_ids)):
            logits_row = logits_row[-completion_ids[i].size(0):, :]
            input_ids_row = input_ids_row[-completion_ids[i].size(0):]
            log_probs = logits_row.log_softmax(dim=-1)
            # print(log_probs.size(), input_ids_row.size())
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # print(completion_ids.size(), completion_ids[0])
        # print( self.processing_class.decode(completion_ids[0]))
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1:
                old_per_token_logps = self._get_per_token_logps(
                    model, prompt_completion_ids, attention_mask, completion_ids, **multimodal_inputs
                )
            ...
            if self.beta == 0.0:
                ...
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, prompt_completion_ids, attention_mask, completion_ids, **multimodal_inputs
                )
            else:
        ...
        completions = [self.processing_class.decode(completion_ids[0], skip_special_tokens=True)]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, completion_ids, **multimodal_inputs)

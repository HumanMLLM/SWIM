...
def _current_ref_for_restore(cwd: Path = PATH_TO_REPO) -> str:
    ...
    return branch if branch else _git("rev-parse", "HEAD", cwd=cwd)
...
def checkout_commit(repo_dir: Path, commit_id: str):
    ...
    Args:
        repo_dir (`Path`): Path to a git repository.
...
if __name__ == "__main__":
    ...
    # Get the commit(s)
    current_head = _current_ref_for_restore(PATH_TO_REPO)
    ...
    for commit in commits:
        with checkout_commit(PATH_TO_REPO, commit):
            commit = _git("rev-parse", "HEAD", cwd=PATH_TO_REPO)

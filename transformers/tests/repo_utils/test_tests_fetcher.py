...
def _parents(repo_dir: Path, commit: str = "HEAD") -> list[str]:
    out = _git(repo_dir, "rev-list", "--parents", "-n", "1", commit)
    parts = out.split()
    return parts[1:]
...
def create_tmp_repo(tmp_dir, models=None):
    ...
    # Initialize a git repo (no GitPython dependency).
    _git(tmp_dir, "init")
    _git(tmp_dir, "checkout", "-b", "main")
    ...
    _git(tmp_dir, "add", "examples", "src", "tests")
    _git(tmp_dir, "commit", "-m", "Initial commit")
    return tmp_dir
...
def commit_changes(filenames, contents, repo_dir: Path, commit_message="Commit"):
    """
    ...
    folder = Path(repo_dir)
    for filename, content in zip(filenames, contents):
        with open(folder / filename, "w") as f:
            f.write(content)
    _git(folder, "add", *filenames)
    _git(folder, "commit", "-m", commit_message)
    return _head_sha(folder)
...
class TestFetcherTester(unittest.TestCase):
    def test_checkout_commit(self):
        with tempfile.TemporaryDirectory() as tmp_folder:
            ...
            repo_dir = create_tmp_repo(tmp_folder)
            initial_sha = _head_sha(repo_dir)
            new_sha = commit_changes(BERT_MODELING_FILE, BERT_MODEL_FILE_NEW_DOCSTRING, repo_dir)

            assert _head_sha(repo_dir) == new_sha
            with checkout_commit(repo_dir, initial_sha):
                assert _head_sha(repo_dir) == initial_sha
    ...
    def test_diff_is_docstring_only(self):
        with tempfile.TemporaryDirectory() as tmp_folder:
            tmp_folder = Path(tmp_folder)
            repo_dir = create_tmp_repo(tmp_folder)
            branching_point = _head_sha(repo_dir)
            bert_file = BERT_MODELING_FILE
            commit_changes(bert_file, BERT_MODEL_FILE_NEW_DOCSTRING, repo_dir)
            assert diff_is_docstring_only(repo_dir, branching_point, bert_file)

            commit_changes(bert_file, BERT_MODEL_FILE_NEW_CODE, repo_dir)
            assert not diff_is_docstring_only(repo_dir, branching_point, bert_file)

    def test_get_diff(self):
        with tempfile.TemporaryDirectory() as tmp_folder:
            tmp_folder = Path(tmp_folder)
            repo_dir = create_tmp_repo(tmp_folder)
            initial_commit = _head_sha(repo_dir)
            bert_file = BERT_MODELING_FILE
            commit_changes(bert_file, BERT_MODEL_FILE_NEW_DOCSTRING, repo_dir)
            base = _head_sha(repo_dir)
            assert get_diff(repo_dir, base, _parents(repo_dir, base)) == []
            ...
            commit_changes(bert_file, BERT_MODEL_FILE_NEW_CODE, repo_dir)
            base = _head_sha(repo_dir)
            assert get_diff(repo_dir, base, _parents(repo_dir, base)) == ["src/transformers/models/bert/modeling_bert.py"]

            commit_changes("src/transformers/utils/hub.py", "import huggingface_hub\n\nnew code", repo_dir)
            base = _head_sha(repo_dir)
            assert get_diff(repo_dir, base, _parents(repo_dir, base)) == ["src/transformers/utils/hub.py"]
            assert get_diff(repo_dir, base, [initial_commit]) == [
...
src/transformers/configuration_utils.py
  src/transformers/models/bert/configuration_bert.py
    ...
    src/transformers/models/bert/modeling_bert.py"""
            ...
            buf = io.StringIO()
            with patch_transformer_repo_path(tmp_folder), redirect_stdout(buf):
            ...
            assert buf.getvalue().strip() in [expected_std_out, expected_std_out_2]
    ...
    def test_infer_tests_to_run(self):
        with tempfile.TemporaryDirectory() as tmp_folder:
            ...
            repo_dir = create_tmp_repo(tmp_folder, models=models)
            _git(repo_dir, "checkout", "-b", "new_model")
            ...
            _git(repo_dir, "add", "src", "tests")
            _git(repo_dir, "commit", "-m", "Add T5 model")

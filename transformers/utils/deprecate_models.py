...
def _git(*args: str, cwd: Path = REPO_PATH, check: bool = True) -> str:
...
def _rel(path: Path) -> str:
    return os.path.relpath(str(path), str(REPO_PATH))
...
def move_model_files_to_deprecated(model):
    ...
    for file in os.listdir(model_path):
        if file == "__pycache__":
            continue
        _git("mv", _rel(model_path / file), _rel(deprecated_model_path / file))
